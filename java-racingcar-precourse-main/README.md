...
-결과를 출력하는 기능
    1. 매 턴의 실행 결과
    2. 최종우승자들을 출력하는 기능
-입력된 결과를 검증하고 유효하지 않다면 에러를 띄우고 다신 입력받는 기능 구현
    1. 자동차 이름을 검증하고 검증이 통과되면 알맞은 데이터 형식으로 반환하는 기능
    2. 자동차 게임의 턴수를 검증하고 검증이 통과되면 알맞은 데이터 형식으로 반환하는 기능
-입력된 데이터를 기반으로 자동차 레이싱 게임 로직 구현
    1. 레이싱 게임 시작 전 필요한 데이터를 입력받는다.
    2. 입력받은 데이터를 검증한다. 검증 통과되면 레이싱을 시작하고 아니면 다시 입력받는다
    3. 레이싱 게임을 진행한다
    4. 우승자를 판별한다
    5. 우승자를 출력한다